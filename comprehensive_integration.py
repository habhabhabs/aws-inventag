...
class ComprehensiveIntegration:
    ...
    def _test_ai_predictions(self):
        ...
        try:
            ...
            if sample_resources:
                ...
                print(f"✅ AI predictions complete: {len(predicted_resources)} predictions generated")
                
                return {
                    "source_resources": len(sample_resources),
                    "predicted_resources": len(predicted_resources),
                    "prediction_rate": len(predicted_resources) / len(sample_resources) if sample_resources else 0
                }
            else:
                print("ℹ️  No source resources found for AI prediction testing")
                return {"source_resources": 0, "predicted_resources": 0, "prediction_rate": 0}
    ...
    def _validate_integration(self):
        ...
        try:
            ...
            if test_result:
                ...
                validation_results["components_tested"] += 1
                validation_results["issues"].append("AWS managed filtering: Not filtering correctly")
        ...
        success_rate = validation_results["components_passed"] / validation_results["components_tested"] if validation_results["components_tested"] > 0 else 0
        
        print(f"✅ Integration validation complete: {validation_results['components_passed']}/{validation_results['components_tested']} components working ({success_rate:.1%})")
    ...
    def _generate_final_report(self):
        ...
        if "tests" in self.results and "error" not in self.results["tests"]:
            ...
            print(f"🧪 System Tests: {test_success_rate:.1%} success rate ({self.results['tests']['passed']}/{self.results['tests']['total']})")
        ...
        if "performance" in self.results and "error" not in self.results["performance"]:
            ...
            print(f"⚡ Performance: {throughput:.1f} resources/sec across {self.results['performance']['services_tested']} services")
        else:
        ...
        if "filtering" in self.results and "error" not in self.results["filtering"]:
            ...
            print(f"🔍 Filtering: {services_analyzed} services analyzed, {false_positives} false positives")
        else:
        ...
        if "validation" in self.results:
            ...
            validation_rate = validation["components_passed"] / validation["components_tested"] if validation["components_tested"] > 0 else 0
            print(f"✅ Integration: {validation_rate:.1%} components working ({validation['components_passed']}/{validation['components_tested']})")
