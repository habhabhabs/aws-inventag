...
class OptimizedAWSDiscovery(IntelligentAWSDiscovery):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_service):
                ...
                try:
                    ...
                    self.logger.info(f"Parallel discovery: {service} found {len(resources)} resources")
                except Exception as e:
    ...
    def _discover_services_sequential(self) -> List[StandardResource]:
        ...
        for service in self.priority_services:
            try:
                ...
                all_resources.extend(resources)
                self.logger.info(f"Sequential discovery: {service} found {len(resources)} resources")
            except Exception as e:
    ...
    def _intelligent_deduplication(self, resources: List[StandardResource]) -> List[StandardResource]:
