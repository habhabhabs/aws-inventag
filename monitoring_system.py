    ...
    def _monitoring_loop(self):
        ...
        while self.monitoring_active:
            try:
                ...
                # Log cycle summary
                self.logger.info(f"Discovery cycle complete: {cycle_data['total_resources']} resources in {cycle_duration:.2f}s")
...
This is an automated alert from the AWS Discovery monitoring system.
            ...
            server.starttls()
            server.login(self.config["email_alerts"]["username"], self.config["email_alerts"]["password"])
            
            text = msg.as_string()
            server.sendmail(self.config["email_alerts"]["username"], self.config["email_alerts"]["recipients"], text)
            server.quit()
    ...
    def _attempt_auto_remediation(self, alert):
        ...
        if alert["type"] == "performance" and alert["severity"] == "warning":
            ...
            if hasattr(self.discovery, 'max_workers') and self.discovery.max_workers > 1:
                ...
                self.logger.info(f"🔧 Auto-remediation: Reduced parallel workers to {self.discovery.max_workers}")
    ...
    def generate_monitoring_report(self):
        ...
        for service, data in status['services_status'].items():
            ...
            print(f"  {status_emoji} {service.upper()}: {data['resource_count']} resources ({data['duration']:.2f}s)")
