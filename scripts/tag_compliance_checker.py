...
class ComprehensiveTagComplianceChecker:
    ...
    def _load_tag_policy(self) -> Dict[str, Any]:
        ...
        if not self.config_file:
            print(
                f"{Fore.RED}WARNING: No configuration file specified. Only checking for completely untagged resources."
            )
            print(
                f"{Fore.RED}It is recommended to provide a configuration file with at least 1 required tag key.{Style.RESET_ALL}"
            )
        ...
        try:
            ...
            if len(config["required_tags"]) == 0:
                print(
                    f"{Fore.RED}WARNING: Configuration file contains no required tags. Consider adding at least 1 required tag key.{Style.RESET_ALL}"
                )
    ...
    def _discover_via_config_service(self):
        ...
        for region in self.regions:
            try:
                        ...
                        for resource_type in supported_types:
                            try:
                                    ...
                                    for resource in page.get("resourceIdentifiers", []):
                                        ...
                                        try:
                                            ...
                                            if config_item.get("configurationItems"):
                                                ...
                                                resource_info = {
                                                    "service": resource_type.split(
                                                        "::"
                                                    )[1],
                                                    "type": resource_type.split("::")[
                                                        2
                                                    ],
                                                    "region": region,
                                                    ...
                                                    "arn": item.get(
                                                        "arn",
                                                        f"arn:aws:{resource_type.split('::')[1].lower()}:{region}:{item.get('accountId', '')}:{resource['resourceId']}",
                                                    ),
                                                    "tags": tags,
                                                    "discovered_via": "AWSConfig",
                                                    "discovered_at": datetime.utcnow().isoformat(),
                                                    "resource_type": resource_type,
                                                    "configuration_state": item.get(
                                            ...
                                            self.logger.debug(
                                                f"Could not get config for {resource_type} {resource.get('resourceId')}: {e}"
                                            )
                                            continue
                                ...
                                if "ResourceNotDiscoveredException" not in str(e):
                                    self.logger.debug(
                                        f"Could not discover {resource_type} in {region}: {e}"
                                    )
                                continue
                ...
                self.logger.debug(
                    f"Unexpected error in Config discovery for {region}: {e}"
                )
    ...
    def _discover_via_cloudtrail(self):
        ...
        try:
            cloudtrail = self.session.client(
                "cloudtrail", region_name="us-east-1"
            )  # CloudTrail events are global

            # Look for CreateXXX events in the last 7 days
            end_time = datetime.utcnow()
            start_time = (
                end_time.replace(day=end_time.day - 7)
                if end_time.day > 7
                else end_time.replace(month=end_time.month - 1, day=30)
            )
                        ...
                        try:
                            ...
                            for resource in resources:
                                ...
                                if resource_name and resource_type:
                                    ...
                                    resource_info = (
                                        self._construct_resource_from_cloudtrail(
                                            event, resource
                                        )
                                    )
                                    if resource_info:
                            ...
                            self.logger.debug(
                                f"Could not process CloudTrail event {event_name}: {e}"
                            )
                            continue
    ...
    def _discover_apigateway_resources(self, region: str):
        ...
        try:
            ...
            self.logger.debug(
                f"Failed to discover API Gateway resources in {region}: {e}"
            )
    ...
    def _discover_elasticache_resources(self, region: str):
        ...
        try:
                ...
                for cluster in page.get("CacheClusters", []):
                    ...
                    try:
                        tags_response = elasticache.list_tags_for_resource(
                            ResourceName=cluster["CacheClusterArn"]
                        )
                        tags = {
                            tag["Key"]: tag["Value"]
                    ...
                    resource_info = {
                        ...
                        "arn": cluster.get(
                            "CacheClusterArn",
                            f"arn:aws:elasticache:{region}:{self.session.get_credentials().access_key}:cluster:{cluster['CacheClusterId']}",
                        ),
                        "tags": tags,
                        "discovered_via": "ServiceSpecificAPI",
                        "discovered_at": datetime.utcnow().isoformat(),
            ...
            self.logger.debug(
                f"Failed to discover ElastiCache resources in {region}: {e}"
            )
        ...
        try:
            ...
            for page in paginator.paginate():
                for topic in page.get("Topics", []):
                    ...
                    # Get tags
                    try:
                        tags_response = sns.list_tags_for_resource(
                ...
                for queue_url in page.get("QueueUrls", []):
                    try:
                        ...
                        resource_info = {
                            ...
                            "discovered_at": datetime.utcnow().isoformat(),
                            "queue_url": queue_url,
                            "message_retention": attrs["Attributes"].get(
                        ...
                        self.logger.debug(
                            f"Could not get details for SQS queue {queue_url}: {e}"
                        )
                        continue
            ...
            for page in paginator.paginate():
                for key in page.get("Keys", []):
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"Could not get details for KMS key {key_id}: {e}"
                        )
                        continue
        ...
        try:
            ...
            try:
                ...
                for domain in domains.get("DomainNames", []):
                    ...
                    try:
                        # Get domain details
                        domain_details = opensearch.describe_domain(
                        ...
                        # Get tags
                        try:
                            tags_response = opensearch.list_tags(
                        ...
                        self.logger.debug(
                            f"Could not get details for OpenSearch domain {domain_name}: {e}"
                        )
                        continue
                ...
                for domain in domains.get("DomainNames", []):
                    ...
                    try:
                        ...
                        resource_info = {
                            ...
                            "discovered_via": "ServiceSpecificAPI",
                            "discovered_at": datetime.utcnow().isoformat(),
                            "elasticsearch_version": domain_status.get(
                        ...
                        self.logger.debug(
                            f"Could not get details for Elasticsearch domain {domain_name}: {e}"
                        )
                        continue

        except ClientError as e:
            self.logger.debug(
                f"Failed to discover Elasticsearch/OpenSearch resources in {region}: {e}"
            )
    ...
    def _extract_resource_name(
        self, tags: Dict[str, str], additional_info: Dict[str, Any]
    ) -> str:
    ...
    def _construct_resource_from_cloudtrail(
        self, event: Dict[str, Any], resource: Dict[str, Any]
        ...
        try:
            ...
            return {
                ...
                "arn": (
                    ...
                    else f"arn:aws:{service.lower()}:{event.get('AwsRegion', '')}:{event.get('RecipientAccountId', '')}:{resource_name}"
                ),
            ...
            self.logger.debug(
                f"Could not construct resource from CloudTrail event: {e}"
            )
            return None
    ...
    def _deduplicate_resources(self):
        ...
        self.logger.info(
            f"After deduplication: {len(self.all_resources)} unique resources"
        )
    ...
    def _generate_summary(self):
        ...
        self.compliance_results["summary"] = {
            ...
            "compliance_percentage": (
                (len(self.compliance_results["compliant"]) / total_resources * 100)
    ...
    def print_summary(self):
        ...
        print(
            f"Tag Policy File: {summary.get('tag_policy_file', 'None (checking untagged only)')}"
        )
        ...
        print(
            f"{Fore.RED}Non-Compliant: {summary['non_compliant_count']}{Style.RESET_ALL}"
        )
        ...
        if summary.get("service_breakdown"):
            ...
            print(
                f"{'Service':<20} {'Total':<8} {'Compliant':<10} {'Non-Compliant':<15} {'Untagged':<10} {'Rate':<6}"
            )
            print("-" * 75)
            ...
            for service, stats in sorted(summary["service_breakdown"].items()):
                ...
                color = (
                    Fore.GREEN if rate > 80 else Fore.YELLOW if rate > 50 else Fore.RED
                )
                print(
                    f"{service:<20} {stats['total']:<8} {stats['compliant']:<10} {stats['non_compliant']:<15} {stats['untagged']:<10} {color}{rate:.1f}%{Style.RESET_ALL}"
                )
            print()
        ...
        if self.tag_policy and "required_tags" in self.tag_policy:
            ...
            if missing_tags:
                ...
                for tag, count in sorted(
                    missing_tags.items(), key=lambda x: x[1], reverse=True
                )[:10]:
    ...
    def save_report(self, filename: str, format_type: str = "json"):
        ...
        report_data = {
            "summary": self.compliance_results["summary"],
            "compliant_resources": [
                item["resource"] for item in self.compliance_results["compliant"]
            ],
            "non_compliant_resources": self.compliance_results["non_compliant"],
            "untagged_resources": [
                item["resource"] for item in self.compliance_results["untagged"]
            ],
    ...
    def upload_to_s3(self, bucket_name: str, key: str, format_type: str = "json"):
        ...
        try:
            ...
            self.logger.info(
                f"Comprehensive compliance report uploaded to s3://{bucket_name}/{key}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Comprehensive AWS Tag Compliance Checker - Discovers ALL AWS Resources"
    )
    parser.add_argument(
        "--config", "-c", help="Tag policy configuration file (JSON or YAML)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="comprehensive_tag_compliance_report",
    ...
    try:
        ...
        print(
            f"{Fore.CYAN}Initializing Comprehensive AWS Tag Compliance Checker...{Style.RESET_ALL}"
        )
        ...
        print(
            f"{Fore.CYAN}Discovering ALL AWS resources across your account...{Style.RESET_ALL}"
        )
        ...
        print(
            f"{Fore.GREEN}Successfully discovered {len(all_resources)} resources{Style.RESET_ALL}"
        )
        ...
        if (
            results["summary"]["non_compliant_count"] > 0
            or results["summary"]["untagged_count"] > 0
        ):
            print(
