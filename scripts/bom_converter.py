...
class BOMConverter:
        ...
        for resource in self.data:
            if isinstance(resource, dict):
                ...
                # Fix CloudWatch alarm types (they all have generic "alarm" as ID)
                    ...
                    service == "CLOUDWATCH" and res_type == "Alarm" and ":alarm:" in arn
                ):
                    # Extract alarm name from ARN and use it as a more unique identifier
                    alarm_name = (
                        arn.split(":alarm:")[-1] if ":alarm:" in arn else res_type
                    )
        ...
        for resource in self.data:
            if isinstance(resource, dict):
                ...
                if arn and arn.startswith("arn:aws:"):
                    correct_id, correct_name = self._extract_id_name_from_arn(
                        arn, service
                    )
                    ...
                    if correct_name and (
                        not current_name or current_name != correct_name
                    ):
    ...
    def _merge_cross_method_tags(self):
        ...
        if enriched_count > 0:
            print(
                f"Enhanced {enriched_count} AWSConfig resources with inferred tag information"
            )
    ...
    def _apply_tag_patterns(self, patterns):
        ...
        for resource in self.data:
            if (
                ...
                and resource.get("discovered_via") == "AWSConfig"
                and len(resource.get("tags", {}) or {}) == 0
            ):
                ...
                if sr_key in patterns["service_region"]:
                    for tag_key, tag_values in patterns["service_region"][
                        sr_key
                    ].items():
                ...
                if st_key in patterns["service_type"]:
                    for tag_key, tag_values in patterns["service_type"][st_key].items():
                        if (
                            tag_key not in inferred_tags
                        ):  # Don't override service+region tags
                            most_common_value = max(
                                tag_values.items(), key=lambda x: x[1]
                            )
                            if most_common_value[1] >= 2:  # Appears at least twice
                                inferred_tags[tag_key] = most_common_value[0]
                ...
                for tag_key, tag_values in patterns["account_wide"].items():
                    if tag_key not in inferred_tags:
                        most_common_value = max(tag_values.items(), key=lambda x: x[1])
                        total_tagged_resources = len(
                            [r for r in self.data if len(r.get("tags", {}) or {}) > 0]
                        )
                ...
                if inferred_tags:
                    ...
                    resource["tags_inferred"] = (
                        True  # Flag to indicate these are inferred
                    )
                    enriched_count += 1
    ...
    def to_csv(self, output_filename: str):
        ...
        try:
            ...
            print(
                f"CSV export saved to {output_filename} (AWS service included as column)"
            )
    ...
    def _create_summary_sheet(
        self, wb: "Workbook", resources_by_service: Dict[str, List[Dict]]
    ):
        ...
        for service_name in sorted(resources_by_service.keys()):
            service_resources = resources_by_service[service_name]
            resource_types = set(
                item.get("type", "Unknown") for item in service_resources
            )
    ...
    def _create_service_sheet(
        self, wb: "Workbook", service_name: str, service_resources: List[Dict]
    ):
        ...
        for col_num, header in enumerate(sorted_headers, 1):
            ...
            for row_num in range(
                2, min(102, len(service_resources) + 2)
    ...
    def _collect_headers_for_service(
        self, item: Dict[str, Any], headers_set: set, prefix: str = ""
    ):
