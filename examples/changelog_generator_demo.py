...
def create_sample_delta_report():
    ...
    delta_report = DeltaReport(
        ...
        network_changes={"network_impacting_changes": 2, "connectivity_changes": 1},
        impact_analysis={
            "high_impact_services": ["EC2", "RDS"],
            "affected_resource_count": 5,
        },
        change_statistics={
            "changes_by_service": {"EC2": 2, "RDS": 1, "S3": 1, "KMS": 1, "Lambda": 1}
        },
    )
...
def demonstrate_changelog_generation():
    ...
    changelog = generator.generate_changelog(
        delta_report, title="Production Infrastructure Changes - Weekly Review"
    )
...
def demonstrate_advanced_features():
    ...
    for config in configs:
        ...
        changelog = generator.generate_changelog(
            delta_report, title=f"Infrastructure Changes - {config['name']}"
        )
