...
def main():
    ...
    for state in states:
        ...
        print(
            f"   - Compliance: {metadata.get('compliance_status', {}).get('compliance_percentage', 'N/A')}%"
        )
    ...
    print(
        f"   Retention policy: {stats['retention_days']} days, max {stats['max_snapshots']} snapshots"
    )
