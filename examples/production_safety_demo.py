...
def demo_error_handling(manager):
    ...
    result1 = manager.validate_and_monitor_operation(
        service="ec2", operation="describe_instances", operation_func=failing_operation
    )

    print(
        f"     ValueError handling - Error captured: {'✓' if result1.get('error_context') else '✗'}"
    )
    ...
    print(
        f"     Permission error handling - Error captured: {'✓' if result2.get('error_context') else '✗'}"
    )
    print()
...
def demo_compliance_assessment(manager):
    ...
    print(
        f"   Security Validation: {'✓ ACTIVE' if status.security_validation_passed else '✗ ISSUES'}"
    )
    print(
        f"   Production Monitoring: {'✓ ACTIVE' if status.production_monitoring_active else '✗ INACTIVE'}"
    )
