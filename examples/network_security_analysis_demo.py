    ...
    for vpc_id, vpc in vpc_analysis.items():
        ...
        if vpc.subnets:
            ...
            for subnet in vpc.subnets:
                ...
                print(
                    f"    IPs: {subnet.available_ips:,}/{subnet.total_ips:,} available ({subnet.utilization_percentage:.1f}% used)"
                )
...
def print_resource_network_mapping(enriched_resources):
    ...
    for vpc_name, resources in vpc_resources.items():
        ...
        for resource in resources:
            service_icon = {"EC2": "🖥️", "RDS": "🗄️", "Lambda": "⚡", "ELB": "⚖️"}.get(
                resource["service"], "📦"
            )
    ...
    if no_vpc_resources:
        ...
        for resource in no_vpc_resources:
            service_icon = {"S3": "🪣", "IAM": "👤", "CloudFront": "🌐"}.get(
                resource["service"], "📦"
            )
            print(f"  {service_icon} {resource['service']}: {resource['name']}")
...
def main():
    ...
    print(
        '   python -c "from inventag import AWSResourceInventory; from inventag.discovery.network_analyzer import NetworkAnalyzer; inventory = AWSResourceInventory(); resources = inventory.discover_resources(); analyzer = NetworkAnalyzer(); vpc_analysis = analyzer.analyze_vpc_resources(resources)"'
    )
    print()
