...
class PerformanceMonitor:
    ...
    def _generate_benchmark_report(self, results, total_time):
        ...
        for i, (service, data) in enumerate(sorted_results, 1):
            print(f"  {i:2d}. {service.upper():12s} - {data['throughput']:6.1f} resources/sec ({data['avg_time']:.2f}s avg)")
    ...
    def _generate_optimization_recommendations(self, results):
        ...
        if high_resource_services:
            recommendations.append(f"📊 High resource count services: {', '.join(high_resource_services)}")
        ...
        if low_throughput_services:
            recommendations.append(f"⚡ Low throughput services: {', '.join(low_throughput_services)}")
        ...
        if self.cpu_usage and max(self.cpu_usage) > 80:
            recommendations.append("🖥️  High CPU usage detected - consider reducing parallel workers")
