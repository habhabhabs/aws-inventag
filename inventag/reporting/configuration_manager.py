...
class ConfigurationValidator:
    ...
    def validate_configuration_set(self, config_set: ConfigurationSet) -> List[str]:
        ...
        tag_mapping_dict = {
            name: asdict(config) for name, config in config_set.tag_mappings.items()
        }
        errors.extend(self.validate_tag_mappings(tag_mapping_dict))

        # Validate template references
        if (
            config_set.default_template
            and config_set.default_template not in config_set.document_templates
        ):
            errors.append(
                f"Default template '{config_set.default_template}' not found in templates"
            )

        # Validate branding references
        if (
            config_set.default_branding
            and config_set.default_branding not in config_set.branding_configurations
        ):
            errors.append(
                f"Default branding '{config_set.default_branding}' not found in branding configurations"
            )
...
class ConfigurationManager:
    ...
    def get_branding_configuration(
        self, config_name: str, branding_name: Optional[str] = None
    ) -> Optional[AdvancedBrandingConfig]:
    ...
    def _create_default_service_descriptions(
        self,
    ) -> Dict[str, ServiceDescriptionConfig]:
        ...
        return {
            ...
            "S3": ServiceDescriptionConfig(
                ...
                category="Storage",
                resource_types={"Bucket": "Container for objects stored in Amazon S3"},
            ),
    ...
    def _create_default_branding_configurations(
        self,
    ) -> Dict[str, BrandingConfigurationSet]:
