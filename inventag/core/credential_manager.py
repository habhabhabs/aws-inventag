...
class CredentialManager:
    ...
    def load_credential_file(
        self, credential_file: str, password: Optional[str] = None
    ...
    def interactive_credential_setup(
        self, save_to_file: Optional[str] = None, encrypt_file: bool = True
    ) -> List[AccountCredentials]:
        """
        Interactive credential prompt system for secure key input.
        ...
        while True:
            ...
            services_input = input(
                "Specific services to scan (comma-separated, empty for all): "
            ).strip()
            if services_input:
    ...
    def validate_account_credentials(
        ...
        result = CredentialValidationResult(
            is_valid=False, validation_timestamp=datetime.now(timezone.utc).isoformat()
        )
        ...
        try:
            ...
            self.logger.info(
                f"Successfully validated credentials for account {credentials.account_id}"
            )

        except Exception as e:
            result.error_message = str(e)
            self.logger.warning(
                f"Credential validation failed for account {credentials.account_id}: {e}"
            )
        ...
        for credentials in accounts:
            try:
                ...
                try:
                    ...
                    self.logger.info(
                        f"Created role {role_name} in account {credentials.account_id}"
                    )
                except ClientError as e:
                    if e.response["Error"]["Code"] == "EntityAlreadyExists":
                        self.logger.info(
                            f"Role {role_name} already exists in account {credentials.account_id}"
                        )
                    else:
                        raise
    ...
    def _generate_permissions_summary(self, session: boto3.Session) -> Dict[str, Any]:
        ...
        summary["read_only_confirmed"] = summary["ec2_access"] and (
