...
class DiscoveryTester:
    ...
    def test_aws_managed_filtering(self):
        ...
        for service, resource_id, resource_type, should_filter in test_cases:
            ...
            is_filtered = mapper._is_aws_managed_resource(raw_data, service, resource_id, resource_type)
    ...
    def test_ai_predictions(self):
        ...
        try:
            ...
            predicted_resources = [r for r in all_resources if hasattr(r, 'confidence_score') and r.confidence_score < 0.8]
            ...
            if predicted_resources:
                ...
                for pred in predicted_resources[:3]:  # Show first 3
                    print(f"    🔮 Predicted: {pred.service}:{pred.resource_type}:{pred.resource_id} (confidence: {pred.confidence_score:.2f})")
            else:
    ...
    def test_performance_comparison(self):
        ...
        for service in test_services:
            try:
                ...
                print(f"    🚀 Optimized: {len(optimized_resources)} resources in {optimized_time:.2f}s")
                print(f"    🧠 Intelligent: {len(intelligent_resources)} resources in {intelligent_time:.2f}s")
                
                if optimized_time < intelligent_time:
                    speedup = intelligent_time / optimized_time
                    print(f"    ✅ Optimized is {speedup:.1f}x faster")
                else:
                    print(f"    ⚠️  Intelligent was faster by {optimized_time / intelligent_time:.1f}x")
                
                # Compare resource counts
                if len(optimized_resources) >= len(intelligent_resources):
                    print(f"    ✅ Optimized found {len(optimized_resources) - len(intelligent_resources)} more resources")
                else:
                    print(f"    ⚠️  Optimized found {len(intelligent_resources) - len(optimized_resources)} fewer resources")
    ...
    def test_service_coverage(self):
        ...
        services_to_test = [
            "s3", "ec2", "iam", "lambda", "cloudfront", 
            "route53", "rds", "cloudwatch", "logs"
        ]
    ...
    def run_comprehensive_test(self):
        ...
        for test_name, result in test_results.items():
            if isinstance(result, bool):
                ...
                print(f"📊 {test_name.replace('_', ' ').title()}: {sum(result.values())} total resources")
