...
class TestDeltaDetector:
    ...
    def sample_new_resources(self):
        ...
        return [
            ...
            },
            {
                ...
                "encryption": {
                    "enabled": True,
                    "kms_key": "arn:aws:kms:us-east-1:123456789012:key/12345",
                },  # Modified encryption
                "public_access": False,
            },
    ...
    def test_initialization(self):
        ...
        assert detector.ignore_metadata_fields == [
            "last_seen",
            "discovery_timestamp",
            "scan_time",
            "metadata",
    ...
    def test_detect_added_resources(
        ...
        assert (
            added_resources[0].resource_arn
    ...
    def test_compare_resources_simple_changes(self, delta_detector):
        ...
        compliance_change = next(
            (c for c in changes if c.attribute_path == "compliance_status"), None
        )
    ...
    def test_compare_resources_nested_objects(self, delta_detector):
        ...
        enabled_change = next(
            (c for c in changes if c.attribute_path == "encryption.enabled"), None
        )
    ...
    def test_compare_resources_list_changes(self, delta_detector):
        ...
        new_resource = {
            ...
            "security_groups": [
                "sg-12345",
                "sg-abcde",
            ],  # Removed sg-67890, added sg-abcde
        }
    ...
    def test_categorize_attribute_change(self, delta_detector):
        ...
        assert (
            delta_detector._categorize_attribute_change("security_groups", [], [])
            == ChangeCategory.SECURITY
        )
        assert (
            delta_detector._categorize_attribute_change("iam_role", None, "role")
            == ChangeCategory.SECURITY
        )
        assert (
            delta_detector._categorize_attribute_change("encryption", {}, {})
            == ChangeCategory.SECURITY
        )

        # Network-related attributes
        assert (
            delta_detector._categorize_attribute_change("vpc_id", "vpc-1", "vpc-2")
        ...
        assert (
            delta_detector._categorize_attribute_change("compliance_violations", [], [])
            == ChangeCategory.COMPLIANCE
        )

        # Configuration (default)
        assert (
            delta_detector._categorize_attribute_change("state", "running", "stopped")
            == ChangeCategory.CONFIGURATION
        )
        assert (
            delta_detector._categorize_attribute_change("name", "old-name", "new-name")
            == ChangeCategory.CONFIGURATION
        )
    ...
    def test_determine_attribute_severity(self, delta_detector):
        ...
        # Critical severity
        assert (
            delta_detector._determine_attribute_severity("security_groups", [], [])
            == ChangeSeverity.CRITICAL
        )
        assert (
            delta_detector._determine_attribute_severity("encryption", {}, {})
            == ChangeSeverity.CRITICAL
        )

        # High severity
        assert (
            delta_detector._determine_attribute_severity(
                "compliance_status", "compliant", "non-compliant"
            )
            == ChangeSeverity.HIGH
        )
        ...
        # Low severity
        assert (
            delta_detector._determine_attribute_severity("description", "old", "new")
            == ChangeSeverity.LOW
        )
        assert (
    ...
    def test_determine_resource_severity(self, delta_detector):
        ...
        assert (
            delta_detector._determine_resource_severity(iam_resource, ChangeType.ADDED)
            == ChangeSeverity.CRITICAL
        )
        ...
        assert (
            delta_detector._determine_resource_severity(ec2_resource, ChangeType.ADDED)
            == ChangeSeverity.HIGH
        )
        ...
        assert (
            delta_detector._determine_resource_severity(s3_resource, ChangeType.ADDED)
            == ChangeSeverity.MEDIUM
        )
    ...
    def test_detect_compliance_changes(self, delta_detector):
        ...
        old_resource = {"compliance_status": "compliant", "compliance_violations": []}
        ...
        # Check compliance status change
        status_change = next(
            (c for c in compliance_changes if c.attribute_path == "compliance_status"),
            None,
        )
    ...
    def test_assess_network_impact(self, delta_detector):
        ...
        impact = delta_detector._assess_network_impact(
            new_resource, ChangeType.MODIFIED, old_resource
        )
        assert impact is not None
    ...
    def test_detect_changes_comprehensive(
        self, delta_detector, sample_old_resources, sample_new_resources
    ):
