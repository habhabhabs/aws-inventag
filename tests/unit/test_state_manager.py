...
class TestStateManager:
    ...
    def state_manager(self, temp_dir):
        ...
        return StateManager(
            state_dir=f"{temp_dir}/.inventag/state", retention_days=7, max_snapshots=5
        )
    ...
    def test_save_state_with_additional_data(
        self, state_manager, sample_resources, sample_compliance_data
    ):
    ...
    def test_export_state_csv(self, state_manager, sample_resources, temp_dir):
        ...
        exported_file = state_manager.export_state(
            state_id=state_id, export_format="csv", output_file=output_file
        )
