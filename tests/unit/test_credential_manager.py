...
class TestCredentialManager:
    ...
    def test_serialize_account_credentials_with_keyring(self, mock_keyring):
        ...
        mock_keyring.set_password.assert_any_call(
            "inventag", "inventag_account_123456789012", "secret"
        )
        mock_keyring.set_password.assert_any_call(
            "inventag", "inventag_account_123456789012_token", "token"
        )
    ...
    def test_create_session_from_credentials_role_assumption(
    ...
    def test_validate_account_credentials_success(
        ...
        with patch.object(
            credential_manager,
            "_create_session_from_credentials",
    ...
    def test_create_secure_credential_file_unencrypted(
        self, credential_manager, sample_credentials, tmp_path
    ):
        ...
        created_file = credential_manager.create_secure_credential_file(
            sample_credentials, str(output_file), encrypt=False
        )
    ...
    def test_create_secure_credential_file_yaml(
        self, credential_manager, sample_credentials, tmp_path
    ):
    ...
    def test_create_secure_credential_file_encrypted(
        self, mock_getpass, credential_manager, sample_credentials, tmp_path
    ):
        ...
        created_file = credential_manager.create_secure_credential_file(
            sample_credentials, str(output_file), encrypt=True
        )
    ...
    def test_load_credential_file_unencrypted(
        self, credential_manager, sample_credentials, tmp_path
    ):
    ...
    @patch("inventag.core.credential_manager.getpass.getpass")
    def test_load_credential_file_encrypted(
        self, mock_getpass, credential_manager, sample_credentials, tmp_path
    ):
...
class TestCredentialManagerIntegration:
    ...
    def test_interactive_credential_setup_success(
        ...
        mock_validate.return_value = CredentialValidationResult(
            is_valid=True, account_id="123456789012"
        )
    ...
    @patch("inventag.core.credential_manager.getpass.getpass")
    @patch.object(CredentialManager, "validate_account_credentials")
    def test_interactive_credential_setup_validation_failure(
        self, mock_validate, mock_getpass, mock_input
    ):
        ...
        mock_validate.return_value = CredentialValidationResult(
            is_valid=False, error_message="Invalid profile"
        )
