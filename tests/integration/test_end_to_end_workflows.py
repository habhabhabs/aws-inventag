...
class TestEndToEndWorkflows:
    ...
    def test_discovery_to_compliance_workflow(self, temp_workspace, sample_aws_resources, tag_policy_config):
        ...
        result = subprocess.run([
            ...
            '--config', str(policy_file),
            '--output', str(compliance_file),
            '--format', 'json'
        ...
        if result.returncode == 0:
            ...
            assert any(error in result.stderr for error in [
                'ModuleNotFoundError', 'ImportError', 'No module named'
            ])

    def test_compliance_to_bom_workflow(self, temp_workspace, sample_aws_resources, tag_policy_config):
        ...
        compliance_data = {
            ...
            "non_compliant_resources": [
                {**sample_aws_resources[2], "compliance_status": "non_compliant", 
    ...
    def test_full_discovery_compliance_bom_workflow(self, temp_workspace, sample_aws_resources, tag_policy_config):
        ...
        if compliance_result.returncode == 0:
            ...
            if compliance_report_file.exists():
                ...
                bom_result = subprocess.run([
                    sys.executable, 'scripts/bom_converter.py',
                    '--input', str(compliance_report_file),
                    '--output', str(bom_file),
                    '--format', 'excel',
                    '--no-vpc-enrichment'
                ], capture_output=True, text=True, cwd=os.path.join(os.path.dirname(__file__), '..', '..'))
...
class TestMockAWSEnvironment:
    ...
    def comprehensive_aws_resources(self) -> List[Dict[str, Any]]:
        ...
        return [
            # EC2 Resources
            {
                ...
                "arn": "arn:aws:ec2:us-east-1:123456789012:instance/i-1234567890abcdef0",
                "account_id": "123456789012",
                "tags": {"Name": "web-server", "Environment": "production", "Owner": "web-team"}
            },
            {
                "service": "EC2",
                "type": "Volume",
            ...
            # IAM Resources
            {
                ...
                "account_id": "123456789012",
                "tags": {"Environment": "production", "Owner": "security-team", "Role": "access"}
            },
            # CloudFormation Resources
            {
                ...
                "account_id": "123456789012",
                "tags": {"Environment": "production", "Owner": "devops-team", "Role": "infrastructure"}
            }
        ]
    ...
    def test_service_specific_processing(self, temp_workspace, comprehensive_aws_resources):
        ...
        for service in services:
            ...
            result = subprocess.run([
                sys.executable, 'scripts/bom_converter.py',
                '--input', str(service_inventory_file),
                '--output', str(service_bom_file),
                '--format', 'csv',
                '--no-vpc-enrichment'
            ], capture_output=True, text=True, cwd=os.path.join(os.path.dirname(__file__), '..', '..'))
