...
class TestCICDIntegration:
    ...
    def test_notification_generation(self):
        ...
        compliance_summary = {
            ...
            "violations": [{"type": "missing_tag", "count": 10}],
        }
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            mock_doc_gen_instance.generate_bom_documents.return_value = Mock(
                                successful_formats=2, failed_formats=0
                            )
                            ...
                            with patch("boto3.client") as mock_boto3:
                                ...
                                with patch("requests.post") as mock_post:
                                    ...
                                    generator = CloudBOMGenerator(
                                        session=mock_session_instance, config=config
                                    )
    ...
    def test_error_handling_in_cicd(self):
        ...
        config = {"cicd": {"fail_on_errors": True, "retry_attempts": 3}}
