...
class TestPerformanceBenchmarks:
    ...
    def test_network_analyzer_performance(self):
        ...
        with patch.object(analyzer, "_cache_network_info"):
            ...
            ), f"Network analysis took {processing_time:.2f}s, expected < 10.0s"
    ...
    def test_security_analyzer_performance(self):
        ...
        with patch.object(analyzer, "_cache_security_group_info"):
            ...
            assert (
                processing_time < 8.0
            ), f"Security analysis took {processing_time:.2f}s, expected < 8.0s"
    ...
    def test_concurrent_document_generation(self):
        ...
        resource_chunks = [
            resources[i : i + chunk_size] for i in range(0, len(resources), chunk_size)
        ]
        ...
        assert (
            processing_time < 10.0
        ), f"Concurrent processing took {processing_time:.2f}s, expected < 10.0s"
    ...
    def test_memory_usage_scaling(self):
        ...
        for i in range(1, len(memory_usage)):
            ...
            assert (
                ratio < size_ratio * 2
            ), f"Memory usage growing too fast: {ratio:.2f}x vs {size_ratio:.2f}x data"
    ...
    def test_document_generation_performance(self):
        ...
        with patch("inventag.reporting.csv_builder.CSVBuilder.create_bom_csv") as mock_csv:
            ...
            assert (
                processing_time < 5.0
            ), f"Document generation took {processing_time:.2f}s, expected < 5.0s"
